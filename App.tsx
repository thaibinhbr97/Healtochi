...
import { Home, CheckSquare, Mic, BarChart2, Heart, Plus, Cloud } from 'lucide-react';
import { Tab, Task, PetState, MoodLog } from './types';
...
const App: React.FC = () => {
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case Tab.VOICE:
          // This tab actually opens the modal, but if navigated to, show placeholder
          return (
              <div className="flex flex-col items-center justify-center h-full p-6 text-center">
                  <div className="bg-indigo-100 p-6 rounded-full mb-6">
                      <Mic size={48} className="text-indigo-500" />
                  </div>
                  <h2 className="text-2xl font-bold text-slate-700">Voice Companion</h2>
                  <p className="text-slate-500 mt-2">Talk to {pet.name} to feel better, get motivation, or just share your day.</p>
                  <button 
                    onClick={() => setShowVoiceModal(true)}
                    className="mt-8 bg-indigo-600 text-white px-8 py-3 rounded-full font-bold shadow-lg"
                  >
                      Start Conversation
                  </button>
              </div>
